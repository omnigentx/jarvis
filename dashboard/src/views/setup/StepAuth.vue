...
const errors = computed(() => {
  ...
  // Confirm-match only applies when picking a new key; in "confirm existing"
  // mode we hide the second field entirely.
...
const canSubmit = computed(() => {
  ...
  // Only enforce the "confirm matches" rule when the user is *picking* a new
  // key; when confirming an existing one, the second field is hidden.
...
<template>
  <WizardCard
    ...
    :subtitle="keyAlreadyConfigured
      ? 'A master key is already configured on this server (likely from your .env file or a previous setup). Re-enter it below to confirm and continue.'
      : `First, protect your instance with an API key. You'll use this key to log in and to call the API directly.`"
    step-label="Step 1 of 5  ·  Required"
  >
    <div v-if="keyAlreadyConfigured" class="wizard-callout" style="background: rgba(245, 158, 11, 0.08); border-color: rgba(245, 158, 11, 0.35);">
      ...
      <div class="wizard-input-group">
        <input
          ...
          :placeholder="keyAlreadyConfigured
            ...
            : `Min ${MIN_LEN} characters, e.g. letters + digits`"
          v-model="apiKey"
